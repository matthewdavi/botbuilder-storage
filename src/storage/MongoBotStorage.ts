...
import { validateTTLSettings, createSafeConversationId } from "../utils";
...
export class MongoBotStorage implements IBotStorage {
  ...
  constructor(
    ...
    public settings: IMongoBotStorageSettings
  ) {
   ...
   */
  public getData(
    context: IBotStorageContext,
    callback: (err: Error, data: IBotStorageDataHash) => void
  ): void {
    // List of write operations
    const readOps: IMongoReadOperation[] = [];
    const data: IBotStorageDataHash = {};
    const { userId, persistUserData, persistConversationData } = context;
    const conversationId = createSafeConversationId(context.conversationId); //we're adding this to avoid overwrites when conversationId is the same as userId
    if (userId) {
      // Read userData
      if (persistUserData) {
        readOps.push({
    ...
    if (persistConversationData && conversationId) {
      ...
      console.log(conversationId, "read op");
      readOps.push({
        _id: conversationId,
  ...
  public saveData(
    context: IBotStorageContext,
    data: IBotStorageDataHash,
    callback?: (err: Error) => void
  ): void {
    // List of write operations
    const writeOps: IMongoWriteOperation[] = [];
    const { userId, persistUserData, persistConversationData } = context;
    const conversationId = createSafeConversationId(context.conversationId);
    ...
    if (userId) {
      ...
      if (conversationId) {
        ...
        const {
          privateConversationData: d,
          privateConversationDataHash: h
        } = data;
    ...
    if (persistConversationData && conversationId) {
      ...
      console.log(conversationId, "write op");
